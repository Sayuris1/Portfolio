...
vec3 star_layer(vec2 uv, float parallax){
    ...
    for (int y = -1; y <= 1; y++){
        for (int x = -1; x <= 1; x++){
            ...
            float star = star_shape(grid_uv - offset - vec2(random, fract(random * 10.0)) + 0.5, smoothstep(0.8, 0.85, size) * 0.1);
...
void main() {
    ...
    vec3 weight = vec3(uv.y, uv.y, uv.y) * 2.0;
    
    vec3 color = mix(from, to, weight) * 0.2;

    // Cloud effect
    color += smoothstep(0.6, 0.7, fbm(uv * 2.0, pos)) * 0.1;
